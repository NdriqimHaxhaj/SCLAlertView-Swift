...
open class SCLAlertView: UIViewController {
    ...
    fileprivate func setup() {
        ...
        //Gesture Recognizer for tapping inside the Circular Icon
        let circularIconTap = UITapGestureRecognizer(target: self, action: #selector(SCLAlertView.tappedIcon(_:)))
        self.circleView.isUserInteractionEnabled = true
        self.circleView.addGestureRecognizer(circularIconTap)
    ...
    var transferedColorWhenButtonReleased:UIColor?
    func buttonTapDown(_ btn:SCLButton) {
        ...
        transferedColorWhenButtonReleased = btn.backgroundColor
    ...
    func buttonRelease(_ btn:SCLButton) {
        btn.backgroundColor = transferedColorWhenButtonReleased
    }
    ...
    //Dismiss keyboard and Alert when tapping the CircularIcon
    func tappedIcon(_ gestureRecognizer: AnyObject) {
        self.view.endEditing(true)
        hideView()
    }
...
//        let x = (appearance.kCircleHeight - appearance.kCircleIconHeight) / 2
        circleIconView!.frame = CGRect( x: 0, y: 0, width: circleView.frame.width, height: circleView.frame.height)
        circleIconView?.layer.cornerRadius = circleIconView!.bounds.height / 2
        circleIconView?.layer.masksToBounds = true
        circleView.superview?.layer.borderWidth = 1
        circleView.superview?.layer.borderColor = UIColor.init(red: 0, green: 0, blue: 0, alpha: 0.22).cgColor
